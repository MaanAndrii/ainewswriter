...
if ($method === 'POST') {
  ...
  if ($action === 'test_prompt') {
    $sysPrompt  = trim((string)($data['system_prompt'] ?? ''));
    $userPrompt = trim((string)($data['user_prompt']   ?? ''));
    $modelId    = trim((string)($data['model_id']      ?? ''));

    if ($sysPrompt === '' || $userPrompt === '') {
      http_response_code(400);
      echo json_encode(['ok' => false, 'error' => 'system_prompt та user_prompt обов\'язкові']);
      exit;
    }
    ...
    foreach ($settings['models'] ?? [] as $m) {
      if (($m['id'] ?? '') === $modelId) { $modelMeta = $m; break; }
    }
    if (!$modelMeta) {
      http_response_code(404);
      echo json_encode(['ok' => false, 'error' => 'Модель не знайдено: ' . htmlspecialchars($modelId)]);
      exit;
    }

    $provider  = (string)($modelMeta['provider'] ?? 'anthropic');
    $maxTokens = max(256, min(4000, (int)($modelMeta['max_tokens'] ?? 2000)));
    ...
    try {
      $providerObj = match (true) {
        $provider === 'anthropic'                       => new AnthropicProvider($keys['anthropic'] ?? '', false),
        $provider === 'gemini'                          => new GeminiProvider($keys['gemini'] ?? '', false),
        in_array($provider, PROVIDERS_OAI_COMPAT, true) => new OpenAICompatProvider($provider, $keys[$provider] ?? ''),
        default => throw new RuntimeException('Невідомий провайдер: ' . $provider),
      };

      $req     = $providerObj->buildRequest($modelId, $userPrompt, $sysPrompt, false, $maxTokens);
      $payload = json_encode($req['body'], JSON_UNESCAPED_UNICODE);
      ...
      if (!is_array($parsed)) throw new RuntimeException('Невалідна відповідь від API (код ' . $httpCode . ')');

      if ($httpCode !== 200) {
        $errMsg = method_exists($providerObj, 'normalizeError') ? $providerObj->normalizeError($parsed) : null;
        echo json_encode(['ok' => false, 'error' => $errMsg ?? ('HTTP ' . $httpCode . ': ' . substr($response, 0, 200))], JSON_UNESCAPED_UNICODE);
        exit;
      }

      $result = $providerObj->parseResponse($parsed);
      echo json_encode([
        'ok'    => true,
        'text'  => $result['text'],
        'usage' => $result['usage'],
